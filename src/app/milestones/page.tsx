...
const milestones: Milestone[] = [
  {
    ...
    date: 'Now',
    description: 'We are building the core engine. No fluff, just the rails for the future of capital allocation.',
    status: 'in-progress',
    icon: Rocket,
    features: [
  ...
  {
    title: 'Phase 2: The Alpha (Closed Pilot)',
    date: 'Q4 2025',
    description: 'The first deals get done. We validate the model with our Founding Cohort members only.',
    status: 'upcoming',
    icon: Zap,
    features: [
  ...
  {
    title: 'Phase 3: The Beta (Public Access)',
    date: 'Q2 2026',
    description: 'We open the floodgates. The platform goes live for the broader ecosystem.',
    status: 'upcoming',
    icon: CheckCircle2,
    features: [
      'Automated Matching System Live',
      'Full Governance Token Launch',
      'Global Investor Access',
      'Self-Serve Founder Onboarding',
    ],
  },
  {
    title: 'Phase 4: Global Scale',
    date: '2026 & Beyond',
    description: 'We take NartaQ to every major startup hub on the planet. Capital without borders.',
    status: 'upcoming',
    icon: Globe,
    features: [
      'Multi-Language Support',
      'Cross-Border Legal Automation',
      'Decentralized Community Governance',
      'The New Standard for Funding',
    ],
  },
]
    ...
    >
      ...
      {index < milestones.length - 1 && (
        <div className='absolute left-6 top-20 md:top-24 bottom-0 w-0.5 bg-gradient-to-b from-[#a98b5d] to-transparent hidden md:block'>
          <div
            className={`absolute inset-0 bg-gradient-to-b from-[#a98b5d] to-transparent transition-all duration-1000 origin-top ${isVisible ? 'scale-y-100' : 'scale-y-0'
              }`}
          />
        </div>
      )}
      ...
      <div className='flex gap-6 max-md:flex-col'>
        ...
        <div className='hidden md:flex flex-col items-center flex-shrink-0'>
          <div
            className={`w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${milestone.status === 'completed'
          ...
          >
            <Icon className={`w-6 h-6 ${milestone.status === 'completed'
              ? 'text-green-400'
              : milestone.status === 'in-progress'
                ? 'text-[#a98b5d]'
                : 'text-gray-400'
...
const CohortUrgencyBanner = () => {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border ${isFull
      ...
      <div className='relative z-10'>
        <div className='flex items-start gap-4 mb-6'>
          <div className={`p-3 rounded-full ${isFull ? 'bg-red-500/20' : isNearCapacity ? 'bg-orange-500/20' : 'bg-[#a98b5d]/20'
            }`}>
            {isFull ? (
        ...
        <div className='mb-6'>
          ...
          <div className='h-3 bg-gray-800 rounded-full overflow-hidden'>
            <div
              className={`h-full transition-all duration-1000 ease-out ${isFull
