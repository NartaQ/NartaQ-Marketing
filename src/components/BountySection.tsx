...
export default function BountySection() {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'lock': return <Lock className="w-8 h-8 text-[#a98b5d]" />
      case 'dollarsign': return <DollarSign className="w-8 h-8 text-[#a98b5d]" />
      default: return <FileText className="w-8 h-8 text-[#a98b5d]" />
    }
  };
      ...
      <div className="mt-20 reveal-up">
        ...
        <div className="flex justify-center items-center space-x-4 max-w-4xl mx-auto">
          {[
            ...
            { step: '02', label: 'Curated Match', desc: 'Vetted providers apply' },
            { step: '03', label: 'Secure Delivery', desc: 'Milestone protection' },
            { step: '04', label: 'Auto Payout', desc: 'On acceptance' }
          ].map((item, i) => (
            <div key={i} className="flex-1 text-center">
              <div className="premium-glass rounded-2xl p-6 mb-3 elite-hover">
                <div className="text-2xl font-light text-[#a98b5d] mb-2">{item.step}</div>
                <div className="font-medium text-[#dcd7ce] text-sm mb-1">{item.label}</div>
