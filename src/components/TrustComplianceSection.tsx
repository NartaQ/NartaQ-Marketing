...
export default function TrustComplianceSection() {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'shield': return <Shield className="w-10 h-10 text-[#a98b5d]" />
      case 'lock': return <Lock className="w-10 h-10 text-[#a98b5d]" />
      case 'scale': return <Scale className="w-10 h-10 text-[#a98b5d]" />
      default: return <Shield className="w-10 h-10 text-[#a98b5d]" />
    }
  };

  return (
    <section id='trust-compliance' className='flex w-full flex-col items-center justify-center p-[2%] py-32 relative'>
      ...
      <div className='max-w-6xl mx-auto text-center space-y-8 relative z-10 reveal-up'>
        ...
        <div className="space-y-6">
          ...
          <h2 className='text-4xl md:text-5xl font-light text-[#dcd7ce]'>
            <span className="font-medium text-[#a98b5d] ">Trust & compliance</span> by design
          </h2>
          ...
          <p className='text-xl text-[#dcd7ce] font-light leading-relaxed max-w-4xl mx-auto'>
            Enterprise-grade KYC/KYB verification, NDA‑gated workflows, IP protection, 
            EU data residency, comprehensive audit logs, and professional dispute resolution. 
            <br /><br />
            <span className="text-[#a98b5d]/80">
              Specialized bounty controls include immutable briefs, timestamped submissions, 
              duplicate prevention, and private bounties for sensitive work.
            </span>
          </p>
        </div>
      </div>

      <div className='grid grid-cols-1 lg:grid-cols-3 gap-8 mt-20 max-w-7xl w-full px-4 reveal-up'>
        {trustFeatures.map((card, i) => (
          <div key={i} className='group premium-glass elite-hover rounded-3xl p-8 border border-[#a98b5d]/20  relative overflow-hidden'>
            {/* Card background gradient */}
            <div className={`absolute inset-0 bg-gradient-to-br ${card.color} opacity-50 rounded-3xl`}></div>
            ...
            <div className="relative z-10">
              ...
              <h3 className='text-2xl font-medium text-[#dcd7ce] mb-4'>{card.title}</h3>
              
              <div className="w-12 h-0.5 bg-[#a98b5d] mb-4"></div>
              
              <p className='text-[#dcd7ce] leading-relaxed font-light'>{card.desc}</p>
